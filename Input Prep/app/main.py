...
from app.services.layer0_client import get_layer0_client, scan_with_layer0
...
async def startup_event():
    ...
    # Check Layer-0 integration
    layer0_client = get_layer0_client()
    if layer0_client.enabled:
        health = await layer0_client.health_check()
        logger.info(f"Layer-0 integration: enabled, status={health.get('status', 'unknown')}")
    else:
        logger.info("Layer-0 integration: disabled")
...
async def prepare_text_input(
    ...
    try:
        ...
        with RequestLogger(request_id, logger) as req_logger:
            ...
            # Step 4.5: Layer-0 Security Scan (CRITICAL SECURITY CHECK)
            step_start = time.time()
            logger.info(f"[{request_id}] ===== LAYER-0 SECURITY SCAN =====")
            
            # Scan with Layer-0
            layer0_allowed, layer0_result, layer0_error = await scan_with_layer0(
                user_input=normalized_user,
                external_chunks=normalized_external,
                fail_open=True  # Allow on error (configurable)
            )
            
            step_times["layer0_scan"] = (time.time() - step_start) * 1000
            
            if layer0_result:
                logger.info(f"[{request_id}] Layer-0 status: {layer0_result.status}")
                logger.info(f"[{request_id}] Layer-0 scan time: {layer0_result.processing_time_ms:.2f}ms")
                logger.info(f"[{request_id}] Layer-0 audit token: {layer0_result.audit_token[:16]}...")
                
                if layer0_result.rule_id:
                    logger.warning(
                        f"[{request_id}] Layer-0 matched rule: {layer0_result.rule_id}, "
                        f"severity={layer0_result.severity}, dataset={layer0_result.dataset}"
                    )
            else:
                logger.info(f"[{request_id}] Layer-0: {layer0_error or 'disabled'}")
            
            # Block if Layer-0 rejected the input
            if not layer0_allowed:
                logger.error(f"[{request_id}] ❌ INPUT BLOCKED BY LAYER-0 ❌")
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
                    detail={
                        "error": "Input blocked by security filter",
                        "reason": layer0_result.status if layer0_result else "security_violation",
                        "rule_id": layer0_result.rule_id if layer0_result else None,
                        "severity": layer0_result.severity if layer0_result else None,
                        "audit_token": layer0_result.audit_token if layer0_result else None
                    }
                )
            
            logger.info(f"[{request_id}] ✓ Layer-0 security check PASSED")
